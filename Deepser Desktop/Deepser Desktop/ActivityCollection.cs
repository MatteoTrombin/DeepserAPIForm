...
{
    public partial class ActivityCollection : Form
    {
        ...
        public List<string> listpar = new List<string>();
        public int i = 0;
        public int j = 0;
        public Dictionary<string, object> dict = new Dictionary<string, object>();
        public Dictionary<string, string> dictoperation = new Dictionary<string, string>
        {
        { "EqualTo", "eq" },
        { "NotEqualTo", "neq" },
        { "EqualsAnyOf", "in" },
                ...
                try
                {
                    ...
                    foreach (DataGridViewRow row in dataGridResult.Rows)
                    {
                        if (row.Cells.Count > 0)
                        {
                            ...
                            if (value != null)
                            {
                                await entity.DeleteActivity(Int32.Parse(textIdField.Text), Int32.Parse(value.ToString()));
                            }
                            else
                            {
                                continue;
        ...
        private async void ButtonBack_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                var list = SqliteDataAccess.GetLastInsertCollAct();
        ...
        private void ButtonUpdate_Click(object? sender, EventArgs e)
        {
            try
            {
                var activityId = dataGridResult.Rows[0].Cells[0].Value.ToString();
                UpdateActivity updateAct = new UpdateActivity();
                updateAct.selectEntity.SelectedItem = selectEntity.SelectedItem;
                updateAct.textId.Text = textIdField.Text;
                updateAct.textActivityID.Text = activityId;

                var data = updateAct.dataGrid;
                int i = 0;
        ...
        private async void ButtonDelete_Click(object? sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    {
                        MessageBox.Show(await entity.DeleteActivity(Int32.Parse(textIdField.Text), Int32.Parse(dataGridResult.Rows[0].Cells[0].Value.ToString())));
                    }
                    else
                    {
        ...
        {
            if (selectEntity.SelectedIndex != -1 && !string.IsNullOrEmpty(textIdField.Text))
            {
                if (selectEntity.SelectedItem != null)
                {
                    ...
                    try
                    {
                        ...
                        if (entity != null)
                        {
                            ...
                            InsertCollectionActivityHistory((string)selectEntity.SelectedItem, textIdField.Text, filter.filters, parameter.parameters);

                            await collection.LoadMultipleActivity(textIdField.Text);
        ...
        private void InsertCollectionActivityHistory(string entity, string id, Dictionary<string, object> filters, Dictionary<string, object> parameters)
        {
            ...
            SqliteDataAccess.InsertCollectionActivityHistory(entity, Int32.Parse(id), jsonFilters, jsonParameters);
        }
        ...
        internal byte[] ImageToByteArray(System.Drawing.Image image)
        {
        ...
        private void buttonAddFav_Click_1(object sender, EventArgs e)
        {
            ...
            var last = SqliteDataAccess.GetLastInsertCollAct();
            if (areEqual)
            {
                SqliteDataAccess.InsertCollectionActivityFav(last.Entity, last.EntityID, last.Filters, last.Parameters);
                buttonAddFav.Image = Properties.Resources.isFav;
            }
            else
            {
                SqliteDataAccess.DeleteFromFavActColl(last.Entity, last.EntityID.ToString(), last.Filters, last.Parameters);
                buttonAddFav.Image = Properties.Resources.isNotFav;
            }
        }
    }
